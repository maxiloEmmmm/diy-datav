...
func (vq *ViewQuery) QueryBg() *AssetsQuery {
	query := &AssetsQuery{config: vq.config}
...
func (vq *ViewQuery) QueryBlocks() *ViewBlockQuery {
	query := &ViewBlockQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithBg tells the query-builder to eager-load the nodes that are connected to
// the "bg" edge. The optional arguments are used to configure the query builder of the edge.
...
func (vq *ViewQuery) sqlAll(ctx context.Context) ([]*View, error) {
	...
	if query := vq.withBg; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*View)
		for i := range nodes {
			if nodes[i].assets_view == nil {
				continue
			}
			fk := *nodes[i].assets_view
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(assets.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "assets_view" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Bg = n
			}
	...
	if query := vq.withBlocks; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*View)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Blocks = []*ViewBlock{}
		}
		query.withFKs = true
		query.Where(predicate.ViewBlock(func(s *sql.Selector) {
			s.Where(sql.InValues(view.BlocksColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.view_blocks
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "view_blocks" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
...
func (vq *ViewQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(view.Columns...)...).From(t1)
	if vq.sql != nil {
		selector = vq.sql
		selector.Select(selector.Columns(view.Columns...)...)
	}
...
func (vgb *ViewGroupBy) Strings(ctx context.Context) ([]string, error) {
	if len(vgb.fields) > 1 {
		return nil, errors.New("model: ViewGroupBy.Strings is not achievable when grouping more than 1 field")
	}
	var v []string
	if err := vgb.Scan(ctx, &v); err != nil {
		return nil, err
...
func (vgb *ViewGroupBy) String(ctx context.Context) (_ string, err error) {
	...
	case 0:
		err = &NotFoundError{view.Label}
	default:
		err = fmt.Errorf("model: ViewGroupBy.Strings returned %d results when one was expected", len(v))
	}
	return
}
...
func (vgb *ViewGroupBy) Ints(ctx context.Context) ([]int, error) {
	if len(vgb.fields) > 1 {
		return nil, errors.New("model: ViewGroupBy.Ints is not achievable when grouping more than 1 field")
	}
	var v []int
	if err := vgb.Scan(ctx, &v); err != nil {
		return nil, err
...
func (vgb *ViewGroupBy) Int(ctx context.Context) (_ int, err error) {
	...
	case 0:
		err = &NotFoundError{view.Label}
	default:
		err = fmt.Errorf("model: ViewGroupBy.Ints returned %d results when one was expected", len(v))
	}
	return
}
...
func (vgb *ViewGroupBy) Float64s(ctx context.Context) ([]float64, error) {
	if len(vgb.fields) > 1 {
		return nil, errors.New("model: ViewGroupBy.Float64s is not achievable when grouping more than 1 field")
	}
	var v []float64
	if err := vgb.Scan(ctx, &v); err != nil {
		return nil, err
...
func (vgb *ViewGroupBy) Float64(ctx context.Context) (_ float64, err error) {
	...
	case 0:
		err = &NotFoundError{view.Label}
	default:
		err = fmt.Errorf("model: ViewGroupBy.Float64s returned %d results when one was expected", len(v))
	}
	return
}
...
func (vgb *ViewGroupBy) Bools(ctx context.Context) ([]bool, error) {
	if len(vgb.fields) > 1 {
		return nil, errors.New("model: ViewGroupBy.Bools is not achievable when grouping more than 1 field")
	}
	var v []bool
	if err := vgb.Scan(ctx, &v); err != nil {
		return nil, err
...
func (vgb *ViewGroupBy) Bool(ctx context.Context) (_ bool, err error) {
	...
	case 0:
		err = &NotFoundError{view.Label}
	default:
		err = fmt.Errorf("model: ViewGroupBy.Bools returned %d results when one was expected", len(v))
	}
	return
}
...
func (vs *ViewSelect) Strings(ctx context.Context) ([]string, error) {
	if len(vs.fields) > 1 {
		return nil, errors.New("model: ViewSelect.Strings is not achievable when selecting more than 1 field")
	}
	var v []string
	if err := vs.Scan(ctx, &v); err != nil {
		return nil, err
...
func (vs *ViewSelect) String(ctx context.Context) (_ string, err error) {
	...
	case 0:
		err = &NotFoundError{view.Label}
	default:
		err = fmt.Errorf("model: ViewSelect.Strings returned %d results when one was expected", len(v))
	}
	return
}
...
func (vs *ViewSelect) Ints(ctx context.Context) ([]int, error) {
	if len(vs.fields) > 1 {
		return nil, errors.New("model: ViewSelect.Ints is not achievable when selecting more than 1 field")
	}
	var v []int
	if err := vs.Scan(ctx, &v); err != nil {
		return nil, err
...
func (vs *ViewSelect) Int(ctx context.Context) (_ int, err error) {
	...
	case 0:
		err = &NotFoundError{view.Label}
	default:
		err = fmt.Errorf("model: ViewSelect.Ints returned %d results when one was expected", len(v))
	}
	return
}
...
func (vs *ViewSelect) Float64s(ctx context.Context) ([]float64, error) {
	if len(vs.fields) > 1 {
		return nil, errors.New("model: ViewSelect.Float64s is not achievable when selecting more than 1 field")
	}
	var v []float64
	if err := vs.Scan(ctx, &v); err != nil {
		return nil, err
...
func (vs *ViewSelect) Float64(ctx context.Context) (_ float64, err error) {
	...
	case 0:
		err = &NotFoundError{view.Label}
	default:
		err = fmt.Errorf("model: ViewSelect.Float64s returned %d results when one was expected", len(v))
	}
	return
}
...
func (vs *ViewSelect) Bools(ctx context.Context) ([]bool, error) {
	if len(vs.fields) > 1 {
		return nil, errors.New("model: ViewSelect.Bools is not achievable when selecting more than 1 field")
	}
	var v []bool
	if err := vs.Scan(ctx, &v); err != nil {
		return nil, err
...
func (vs *ViewSelect) Bool(ctx context.Context) (_ bool, err error) {
	...
	case 0:
		err = &NotFoundError{view.Label}
	default:
		err = fmt.Errorf("model: ViewSelect.Bools returned %d results when one was expected", len(v))
	}
	return
}
...
func (vs *ViewSelect) sqlQuery() sql.Querier {
	selector := vs.sql
